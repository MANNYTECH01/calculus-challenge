...
serve(async (req)=>{
  ...
  try {
    ...
    const { error: adminProfileError } = await supabaseAdmin.from('profiles').insert({
    ...
    const { error: regularProfileError } = await supabaseAdmin.from('profiles').insert({
    ...
    const { error: adminPanelError } = await supabaseAdmin.from('admin_panel').insert({
      user_id: adminUser.user.id,
      is_admin: true
    });
    if (adminPanelError) {
